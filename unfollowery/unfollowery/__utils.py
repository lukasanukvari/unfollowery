...
"""
The following code is owned by: Yeongbin Jo
...
def find_binary_in_path(filename):
    """
    Searches for a binary named `filename` in the current PATH.
    If an executable is found, its absolute path is returned
    else None.
...
def install(path: str=None):
    ...
    :param path: Flag indicating whether to download to current working directory
    :return: The file path of chromedriver
    """
    chromedriver_filepath = download_chromedriver(path)
